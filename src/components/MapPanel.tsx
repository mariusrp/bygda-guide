...
// VIKTIG: Lag ikonet UTANFOR komponenten så det ikkje vert re-laga kvar gong
const iconCache = new Map<string, L.DivIcon>();

const createSimpleBadge = (category: CategoryKey, isSelected: boolean) => {
    // Bruk cache for å unngå å lage nye ikon heile tida
...
function MapBounds({ places }: { places: Place[] }) {
    ...
    useEffect(() => {
        if (places.length === 0 || hasSetBounds.current) return;
...
export default function MapPanel({
    ...
    // Memoisert markers for å unngå re-render
    const markers = useMemo(() => {
        return places.map((place) => ({
            ...place,
            icon: createSimpleBadge(place.category, place.id === selectedId),
        }));
    }, [places, selectedId]);
            ...
            <Stack spacing={1.5} sx={{ height: "100%" }}>
                ...
                <Box
                    sx={{
                        ...
                        // Deaktiver ALLE hover-effektar og transitions
                        "& .simple-badge-stable": {
                ...
                >
                    <MapContainer
                        ...
                        // Viktig: Deaktiver zoom-animasjon som kan forårsake hopp
                        zoomAnimation={false}
                    >
                        <TileLayer
                        ...
                        {markers.map((place) => (
                            <Marker
                                ...
                                eventHandlers={{
                                    click: (e) => {
                                        // Stop propagation for å unngå side-effektar
